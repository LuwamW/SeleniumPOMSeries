...
public class ElementUtil {
		...
		if (suggList.size() > 0) {
			for (WebElement e : suggList) {
				...
				if (text.length() > 0) {
					...
					System.out.println("blak values -- no suggestions");
					break;
				}
			}
		} else
	 ...
	 * displayed but also have a heightand width that is greater than 0. default
	...
	public WebElement retryingElement(By locator, int timeOut) {
		...
		if (element == null) {
			System.out.println(
					"element is not found...tried for " + timeOut + " secs" + " with the interval of 500 milliseconds");
		}
		return element;
	}
	...
	public void waitForPageLoad(int timeOut) {
		...
		while (System.currentTimeMillis() < endTime) {// while true
